...
typedef struct CRYPTO_INFO_t
{
	...
	unsigned __int8 ks[AES_KS];	/* Primary key schedule (if it is a cascade, it conatins multiple concatenated keys) */
	unsigned __int8 ks2[AES_KS];	/* Secondary key schedule (if cascade, multiple concatenated) for XTS mode. */
