					   ...
					   int cipher)
{
	...
	printf("START DecryptBuffer\n");
	
	// Process all blocks in the buffer
	TC_LARGEST_COMPILER_UINT dataUnitNumber;
	for (dataUnitNumber=0;dataUnitNumber<2;dataUnitNumber++){
		printf("%d' data partition\n",dataUnitNumber);
		startBlock=0;
		endBlock=BLOCKS_PER_XTS_DATA_UNIT;
		...
		// Encrypt the data unit number using the secondary key (in order to generate the first 
		// whitening value for this data unit)
		*((unsigned __int64 *) byteBufUnitNo) = LE64 (dataUnitNumber);
		*((unsigned __int64 *) byteBufUnitNo + 1) = 0;
