...
void EncipherBlock(int cipher, void *data, void *ks)
{
	...
	case TRIPLEDES:		break;//TripleDesEncrypt (data, data, ks, 1); break;	// Deprecated/legacy
#endif
...
void DecipherBlock(int cipher, void *data, void *ks)
{
	...
	case TRIPLEDES:	break;//TripleDesEncrypt (data, data, ks, 0); break;	// Deprecated/legacy
#else
...
{
	return FALSE;// && IsAesHwCpuSupported();
}
...
BOOL EAInitMode (PCRYPTO_INFO ci)
{
	...
	case XTS:
		...
		if (EAInit (ci->ea, ci->km2, ci->ks2) != ERR_SUCCESS)
			return FALSE;
	...
	case LRW:
		...
		case 8:
			/* Deprecated/legacy */
			return Gf64TabInit (ci->km2, &ci->gf_ctx);

		case 16:
			return Gf128Tab64Init (ci->km2, &ci->gf_ctx);
	...
	case INNER_CBC:
		{
				...
				cipher = EAGetNextCipher (cryptoInfo->ea, cipher))
			{
				EncryptBufferCBC ((unsigned __int32 *) buf,
					(unsigned int) len,
					ks,
					(unsigned __int32 *) cryptoInfo->km2,
					(unsigned __int32 *) &cryptoInfo->km2[8],
					0,
					cipher);
